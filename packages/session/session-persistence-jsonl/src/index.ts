...
// Add by MHY: fork 独有，上游 0.1.5 无会话删除原语（delete 需要 rmdir 收尾）
...
/** Add by MHY: fork 独有，上游 0.1.5 无会话删除原语。Whether rmdir failed because the directory still has entries. */
...
class JsonlSessionPersistence extends SessionPersistence {
   ...
   *   `false` when the session never materialized.
   */
  override async delete(id: SessionId, signal?: AbortSignal): Promise<boolean> {
    ...
    signal?.throwIfAborted()
    // The session directory holds the transcript plus any session-owned
    // artifacts; unlink the transcript first, then drop the directory only
    ...
    try {
      ...
      // A non-empty directory means other session-owned artifacts remain, and
      // a concurrent deleter may have removed it first; both are the intended
      // outcome. Every other errno still surfaces.
