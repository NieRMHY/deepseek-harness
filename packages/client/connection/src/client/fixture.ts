...
interface WorkspaceDeleteSessionValue { readonly deleted: true; readonly archivedSessionIds: readonly SessionId[] }
...
function createFixtureWorld(options: FixtureOptions): FixtureWorld {
  ...
  const workspaceApi: FixtureWorkspaceApi = {
    ...
    deleteSession: (request) => {
      ...
      if (archiveIndex !== -1) {
        archivedSessionIds.splice(archiveIndex, 1)
        emitWorkspace({ type: 'archived', archivedSessionIds: [...archivedSessionIds] })
      }
      ...
      emitRemote('api-session/removed', [sessionId])
      return sessionOk({ deleted: true as const, archivedSessionIds: [...archivedSessionIds] })
    },
  }
