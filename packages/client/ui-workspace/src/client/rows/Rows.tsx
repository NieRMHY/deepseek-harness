...
export function ProjectRowItem({ group, onToggle, onCreate, actions, drag, home, t }: {
  ...
  const row = group
  // The synthetic buckets have no workspace title: their labels are dictionary copy.
 ...
 * @param props.onDelete - open the delete-confirmation flow for a session.
...
export function SessionNodeItem({
  ...
  // Add by MHY, archive management: archived rows swap archive for restore and permanent deletion.
  /** Restore an archived session (row menu action; commits without a dialog). */
  onUnarchive?: (id: SessionNode['id']) => void
  /** Open the browser-owned delete-confirmation dialog for this session. */
  ...
  // confirmation dialog. Deletion removes the durable log and does confirm.
  const sessionMenuItems = row.archived === true
    ? [
