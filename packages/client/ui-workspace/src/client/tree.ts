...
// Add by MHY, archive management: the registry-global archive set renders as its own trailing group.
...
export function deriveGroups(
  ...
  for (const g of groupByWorkspace(list, workspaces, archived, view.ungroupedOrder)) {
    ...
    groups.push({
      ...
      sessions: expanded
        ? g.sessions.map(session => sessionNode(session, descendants, false, pendingInteractions))
        : [],
    })
  }
  ...
  if (archivedMembers.length > 0) {
    ...
    groups.push({
      ...
      sessions: expanded
        ? archivedMembers.map(session => sessionNode(session, descendants, true, pendingInteractions))
        : [],
    })
  }
  return groups
...
export function deriveFlat(
  ...
  rows.sort(byRecency)
  return rows.map(session => sessionNode(session, descendants, archived.has(session.id), pendingInteractions))
}
