...
export class WorkspaceManager {
  ...
  /**
   * Unarchive one session, then install the returned full archive set
   * without waiting for the changed frame.
  ...
  /**
   * Delete one session, then install the returned archive set without
   * waiting for the changed frame. The session row itself disappears through
   * the sessions domain's removal path, not through this snapshot.
   * @param sessionId - session to delete.
   * @returns the wire result.
   */
  async deleteSession(sessionId: SessionId): Promise<RpcResult<{ deleted: true; archivedSessionIds: SessionId[] }>> {
