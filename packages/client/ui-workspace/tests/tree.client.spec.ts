...
describe('deriveGroups', () => {
  ...
  it('collects archived sessions into a trailing archive group', () => {
    ...
    // The archived member drops from its group; every archived session —
    // accounted or stray — lands in the trailing archive group.
    ...
    expect(groups[1]!.sessions.map(node => node.archived)).toEqual([true, true])
  })
...
describe('deriveFlat', () => {
  ...
  it('leaves archived sessions out of flat mode', () => {
    ...
    const rows = deriveFlat(list(kept, gone), archived('gone'), noAttention)
